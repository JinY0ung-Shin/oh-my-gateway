...
| `CLAUDE_MAX_BUFFER_SIZE` | Framing limit for ONE JSON message on the Claude CLI's stdout — a tool result is one message, so anything larger fails the whole turn with `sdk_error` (fatal SDK reader abort, not a tool error). The gateway installs its own `16777216` default instead of the SDK's `1048576`; set explicitly to raise or lower it (a value at or below `1048576` is a startup warning). **Unit caveat:** the pinned SDK (0.2.128) counts decoded text *characters*, not UTF-8 bytes, although its error text says "bytes" (upstream anthropics/claude-agent-sdk-python#1165) — multibyte text can occupy up to ~4× the nominal value in memory, so treat this as a framing threshold, not a memory or security quota. Results above the limit still abort the turn; the durable pre-framing tool-result budget is tracked in #185 |
...
- `reasoning.effort`: thinking effort for the session (`low|medium|high|xhigh|max`, Claude backend).
  ...
  **Whether it changes anything is the upstream's business**: on the wire it is Anthropic's
  ...
  fills in only for ids the upstream said nothing about. The entry then carries `effort_levels`,
  and a request for a level outside that list is answered with `400 unsupported_reasoning_effort`
