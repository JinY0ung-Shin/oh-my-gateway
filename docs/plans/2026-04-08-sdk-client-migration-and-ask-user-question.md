...
세션 기반 요청을 `query()`에서 `ClaudeSDKClient`로 전환하고, `can_use_tool` 콜백을 활용하여
...
- `ClaudeSDKClient`는 양방향 통신과 `can_use_tool` 콜백을 지원하여 이 문제를 해결 가능
...
- `can_use_tool` 콜백을 세션과 바인딩하여 등록
- `connect(prompt=None)` 호출 (context manager 아닌 수동 관리)
- `permission_prompt_tool_name="stdio"`는 SDK가 자동 설정
...
**`can_use_tool` 콜백 구현**:
```python
async def _make_can_use_tool(session):
...
스트리밍 중 `can_use_tool` 콜백이 호출되면 (= `session.pending_tool_call`이 세팅되면),
...
2. `session.input_event.set()` → 대기 중인 콜백 해제
...
- 대기 중인 `input_event`가 있으면 set하여 콜백 해제
- 다음 요청 시 새 ClaudeSDKClient 자동 생성

### 세션 만료 (TTL)
- `_cleanup_expired_sessions()`에서 `client.disconnect()` 호출
- 대기 중인 `input_event`가 있으면 set → 콜백이 해제되고 에러 전파
- subprocess graceful shutdown (5초 대기 → SIGTERM → 5초 → SIGKILL)

### can_use_tool 콜백과 스트리밍 동기화
- `can_use_tool` 콜백과 `receive_response()` 순회는 동일 이벤트 루프의 다른 코루틴
- SDK 내부에서 콜백이 호출되면 `receive_response()`는 더 이상 메시지를 yield하지 않음
  (CLI가 tool 실행을 멈추고 permission 응답을 기다리므로)
...
| Test | What |
|------|------|
| `create_client()` unit | Mock ClaudeSDKClient, can_use_tool 콜백 등록 확인 |
| `run_completion_with_client()` unit | Mock client.query + receive_response, 메시지 변환 확인 |
| `can_use_tool` 콜백 unit | AskUserQuestion 감지, Event 대기/해제, 다른 도구 허용 확인 |
...
- `can_use_tool` 콜백으로 AskUserQuestion 처리
