...
When the quota is disabled, `/files/quota` answers from configuration alone (`enabled: false`, `used_bytes: 0`) without walking the workspace. The scan is `O(files)` and occupies a worker from the threadpool that every other file route shares, so a disabled feature must not become the most expensive endpoint in the API. This is the same "no scan unless configured" rule that already governs `/files/upload` and `/files/copy`.
